...
class CommentsControllerTest < ActionController::TestCase
  test 'should create comment' do
    assert_difference('Comment.count') do
      post :create, params: {token: polls(:alices_poll).token, comment: {name: 'name', comment: 'miau', calculation: '0x17'}}
    end
  ...
  test 'should not create incomplete comment' do
    assert_no_difference('Comment.count') do
      post :create, params: {token: polls(:alices_poll).token, comment: {name: 'name', comment: '', calculation: '0x17'}}
    end
  ...
  test 'should not allow comment if not enabled' do
    assert_no_difference('Comment.count') do
      post :create, params: {token: polls(:bobs_poll).token, comment: {name: 'name', comment: 'miau', calculation: '0x17'}}
    end
  ...
  test 'should get destroy confirm comment' do
    get :destroy_confirm, params: {token: polls(:alices_poll).token, id: comments(:one).id}
  ...
  test 'should destroy comment' do
    assert_difference('Comment.count', -1) do
      delete :destroy, params: {token: polls(:alices_poll).token, id: comments(:one).id}
    end
  ...
  test 'token should be required' do
    assert_raises ActiveRecord::RecordNotFound do
      post :create, params: {token: 'guessed token', comment: {name: 'name', comment: 'miau', calculation: '0x17'}}
    end
    assert_raises ActiveRecord::RecordNotFound do
      get :destroy_confirm, params: {token: 'guessed token', id: comments(:one).id}
    end
    assert_raises ActiveRecord::RecordNotFound do
      delete :destroy, params: {token: 'guessed token', id: comments(:one).id}
    end
  end
end
