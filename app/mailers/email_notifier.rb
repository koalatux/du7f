...
class EmailNotifier < ActionMailer::Base
  ...
  def poll_created(poll, request)
    @request = request
    @poll = poll
    mail(from: from(request), to: poll.admin_email_address, subject: "Your new poll#{poll.title_in_quotes}")
  end

  def poll_changed(poll, request)
    @request = request
    @poll = poll
    mail(from: from(request), to: poll.admin_email_address, subject: "Changes at your poll#{poll.title_in_quotes}")
  end

  def poll_deleted(poll, request)
    @request = request
    @poll = poll
    mail(from: from(request), to: poll.admin_email_address, subject: "Removal of your poll#{poll.title_in_quotes}")
  end
  ...
  def participant_created(participant, request)
    ...
    mail(from: from(request), to: participant.poll.admin_email_address, subject: "Activity in your poll#{participant.poll.title_in_quotes}")
  end
